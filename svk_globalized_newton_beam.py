...
# Set OMP_NUM_THREADS=1 before importing Firedrake for performance
import os
...
# Discretization
# Alternative mesh setup (e.g., for a 4x1 beam)
# parser.add_argument("--mesh_x", type=int, default=4, help="Base mesh cells in X")
# parser.add_argument("--mesh_y", type=int, default=1, help="Base mesh cells in Y")
parser.add_argument("--N_base", type=int, default=2, help="Base mesh cells (NxN) for UnitSquareMesh")
...
parser.add_argument("--tol", type=float, default=1e-8, help="Base Newton tolerance (overridden by mesh-dependent tol)")
# Globalisation
...
# Alternative: base_mesh = RectangleMesh(args.mesh_x, args.mesh_y, 4, 1, diagonal='crossed')
...
for i, msh in enumerate(mh):
    ...
    # Define mesh-dependent tolerances (matches svk_beam_param_cont.py)
    loose_tol = 1e-2 / (20**i)  # For intermediate load steps
    strict_tol = 5e-5 / (20**i) # For final load step
    ...
    for step_num, load_val in enumerate(load_steps):
        ...
        for j in range(args.N_Newt):
            ...
            # Robust check: update size AND scaled residual must be small
