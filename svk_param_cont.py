...
parser.add_argument("--LS_Type", type = int, default = 2)
...
# svk Newton solver
def svk_Newt(L, u, du, v, f, N_Newt, tol, LS_Type, N_eval, N_lin, u_Newt = None):
            ...
            # Critical point linesearch (Quasi-Newton)
                ...
                for i in range(N_line):
                    ...
                    if abs(beta_cur - beta_pre) < 1e-5:
                        ...
                        break
                    # print(beta_pre)
                    G_cur = assemble(G_svk(u+beta_cur*du_sol, du_sol, f)) # BCs not needed
                    G_pre = assemble(G_svk(u+beta_pre*du_sol, du_sol, f))
                    N_eval += 2
                    # print(G_cur)
                    # print(G_pre)
...
for i, msh in enumerate(mh):
    ...
    # exit()
    # L_svk1 = L_svk(u, v, alpha*f)
    # svk_Newt(L_svk1, u, du, v, f, N_Newt, tol)
    # u *= 0.5
    # print(W14norm(u))
    for k in range(N_param):
        if k == N_param-1:
            tol = 1e-10/(20**i) # use a high accuracy solver only for the last step
        L_svk1 = L_svk(u, v, alpha*f)
        N_eval += 1
        # Use Newton solver
        if k in range(N_param-1):
            u, N_eval, N_lin = svk_Newt(L_svk1, u, du, v, f, N_Newt, tol, LS_Type, N_eval, N_lin)
        if k == N_param-1:
            ...
            u, N_eval, N_lin = svk_Newt(L_svk1, u, du, v, f, N_Newt, tol, LS_Type, N_eval, N_lin)
            print("Second run")
            L_svk1 = L_svk(u_pre, v, f)
            tol = 1e-8/(20**i)
            u_error[i], N_eval, N_lin = svk_Newt(L_svk1, u_pre, du, v, f, N_Newt, tol, LS_Type, N_eval, N_lin, u_Newt=u)
        alpha += 1/N_param
...
# Converges too quickly. Might not be useful
for i in range(levels):
    x_data = numpy.log(numpy.array(range(len(u_error[i]))) + 1)
    y_data = numpy.log(numpy.abs(numpy.log(u_error[i])))
