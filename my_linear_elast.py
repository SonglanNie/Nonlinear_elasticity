...
parser.add_argument("--uex", type = str, default = "[cos(x)*x*(1 - x)*y*(1 - y), cos(y)*x*(1 - x)*y*(1 - y)]") # other ICs
...
parser.add_argument("--N_Newt", type = int, default = 10)
...
for i, msh in enumerate(mh):
    ...
    for j in range(N_Newt):
        ...
        if sqrt(L2norm(du_sol)**2 + L2norm(grad(du_sol))**2) < 1e-12:
            break
        else:
            u += du_sol
        #print(as_vector(u).array())
